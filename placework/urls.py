...
from placework.views import (  # password_reset,
    AddAddressView,
...
urlpatterns = [
    ...
    path(
        'cadastro/active_email/<email>/<uuid>/',
        active_email,
        name='register_active_email',
    ),
    path(
        'cadastro/sending_email_activation/',
        SedingEmailActivationView.as_view(),
        name='register_email_activation',
    ),
    path('alteracao-dados/<int:pk>/', UpdateView.as_view(), name='update'),
    path(
        'adicionar-endereco/<int:pk>/',
